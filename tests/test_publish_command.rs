...
pub mod test_publish_command {
    ...
    use mashinka::grow::{ISO8601_DATE_FORMAT, ISO8601_DATE_TIME_FORMAT};
    use mashinka::grow::lang::Lang;
    use mashinka::{BIN_NAME, TEST_DRAFT_PATH_ARG_KEY, TEST_DRY_RUN_ARG_KEY, TEST_EMPTY_CONTENT, TEST_POSTS_PATH_ARG_KEY, TEST_AUTHOR, TEST_CONTENT, TEST_DESCRIPTION, TEST_DRAFT_CONTENT, TEST_IMAGE, TEST_KEYWORDS_AS_STRING, TEST_LANG_AS_STRING, TEST_POST_CONTENT_TEMPLATE, TEST_POST_TITLE, TEST_SLUG, TEST_TRANSLATION_TEMPLATE, TEST_TMP_DRAFT_FILE_NAME, TEST_TMP_TRANSLATION_FILE_NAME, TEST_TRANSLATIONS_PATH_ARG_KEY};
    ...
    fn init() -> FixturedData {
        let posts_path = TempDir::new()
            .expect("Can't create tmp dir for posts.");
        ...
        let translation_path = NamedTempFile::new(TEST_TMP_TRANSLATION_FILE_NAME)
            .unwrap();
    ...
    fn test_run_publish_command() {
        ...
        let output = test_bin::get_test_bin(BIN_NAME)
            ...
            .args([
                format!("{}={}", TEST_DRAFT_PATH_ARG_KEY, test_data.draft_path.path().to_str().unwrap()),
                format!("{}={}", TEST_POSTS_PATH_ARG_KEY, posts_path),
                format!("{}={}", TEST_TRANSLATIONS_PATH_ARG_KEY, test_data.translation_path.path().to_str().unwrap()),
            ])
            .output();
        ...
        let expected_post_file_name = format!("{formatted_date}-{}@{}.md", TEST_SLUG, TEST_LANG_AS_STRING);
        ...
        let expected_post_content = process_template(
            TEST_POST_CONTENT_TEMPLATE.to_string(),
            post_template_vars,
        );
